...
func test_cache_keys_carry_the_generation_revision() -> void:
	assert_true(
		WorldDataCache.canonical_key({}).begins_with("generator_revision=2|"),
		"la cache dati non puo riusare chiavi della revisione pre-unificazione"
	)
	assert_true(
		WorldDataCache.build_signature({}).begins_with("generator_revision=2|"),
...
func test_deep_signature_tracks_generated_content_not_only_counts() -> void:
	...
	assert_true(
		baseline_signature.begins_with("layout-v2:"),
...
func test_snapshot_v5_roundtrips_deep_content_and_rejects_v4() -> void:
	...
	assert_eq(int(encoded.get("format_version", -1)), 5,
		"il codec scrive il nuovo formato snapshot")

	var decoded := SNAPSHOT_CODEC.world_data_from_dict(encoded)
	assert_false(decoded.is_empty(), "lo snapshot v5 e decodificato")
	...
	assert_true(
		...
		"un blob v5 con layout alterato e firma stale viene rifiutato"
	)

	var legacy_v4 := encoded.duplicate(true)
	legacy_v4["format_version"] = 4
	assert_true(
		SNAPSHOT_CODEC.world_data_from_dict(legacy_v4).is_empty(),
		"un blob v4 viene rifiutato e dovra essere rigenerato"
	)
...
func _make_layout() -> BiomeEnvironmentLayout:
	...
	layout.obstacle_rotations = [0.0, 1.5707963267949]
