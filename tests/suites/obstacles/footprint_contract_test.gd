...
func test_scalable_obstacle() -> void:
	...
	var kit_id := _manifest.get_biome_rock_cliff_kit_id(&"plains")
	var face_path := _manifest.get_rock_cliff_kit_fallback_path(kit_id, &"wall")
	assert_true(face_path.ends_with("cliff_face_generated_v2.png"), "Plains raised rock and void use the same wall fallback")
