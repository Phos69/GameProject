extends Node2D
class_name IsometricCliffRenderer
...
func _position_sprites() -> void:
	...
	_fit_sprite(
		lip_sprite,
		_edge_target_size(horizontal) * Vector2(1.04, 1.18),
		_edge_position() - _outward_direction() * 2.0
	)
...
func _depth_target_size(horizontal: bool) -> Vector2:
	if horizontal:
		return Vector2(zone_size.x * 1.03, maxf(zone_size.y * 2.6, 96.0))
	return Vector2(maxf(zone_size.x * 2.6, 96.0), zone_size.y * 1.03)

func _edge_target_size(horizontal: bool) -> Vector2:
	if horizontal:
		return Vector2(zone_size.x * 1.02, maxf(zone_size.y * 1.75, 56.0))
	return Vector2(maxf(zone_size.x * 1.75, 56.0), zone_size.y * 1.02)

func _edge_position() -> Vector2:
	var half_size := zone_size * 0.5
	match fall_side:
		&"north":
			return Vector2(0.0, half_size.y * 0.48)
		&"south":
			return Vector2(0.0, -half_size.y * 0.48)
		&"west":
			return Vector2(half_size.x * 0.48, 0.0)
		_:
			return Vector2(-half_size.x * 0.48, 0.0)
...
func _depth_position() -> Vector2:
	...
	match fall_side:
		&"north":
			return Vector2(0.0, -half_size.y * 0.16)
		&"south":
			return Vector2(0.0, half_size.y * 0.16)
		&"west":
			return Vector2(-half_size.x * 0.16, 0.0)
		_:
			return Vector2(half_size.x * 0.16, 0.0)
...
func _draw_seeded_vertical_lines() -> void:
	...
	if _side_is_horizontal():
		for index in range(count):
			...
			var length := maxf(zone_size.y * (0.78 + _seeded_unit(index + 17) * 0.22), 54.0)
		...
		for index in range(count):
			...
			var length := maxf(zone_size.x * (0.78 + _seeded_unit(index + 17) * 0.22), 54.0)
			var end := start + outward * length
			draw_line(start, end, line_color, 2.0, true)
