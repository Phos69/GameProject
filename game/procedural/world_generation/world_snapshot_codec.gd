...
static func world_data_from_dict(data: Dictionary) -> Dictionary:
	...
	# La firma snapshot v7 copre il contenuto profondo dei layout e dei lotti. Oltre al
	# version check,
	...
	if stored_signature.is_empty() or stored_signature != _get_cells_signature(cells):
		return {}
...
static func _get_cells_signature(cells: Array[BiomeCell]) -> String:
