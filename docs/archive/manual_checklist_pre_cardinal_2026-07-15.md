...
Il contratto visuale corrente e `docs/top_down_cardinal_contract.md`. Le
sezioni nominate `ISO-001` o Milestone 4/10 descrivono regressioni storiche e
non devono essere usate per generare nuovi asset; i check correnti sono quelli
`TOPDOWN-001` qui sotto e le suite GUT per area.

## TOPDOWN-001 - proiezione e asset correnti

- La griglia e rettangolare, con strade/passaggi H/V e bordi N/E/S/W.
- Destra aumenta `X`, su diminuisce `Y`; movimento e mira diagonali restano
  disponibili.
- Ground, mappa esplorativa, menu e preview non mostrano pavimenti a rombo o
  assi inclinati.
- Cliff, mesa, edifici e prop possono mostrare facciata sud e volume, ma
  footprint `F9` e collisioni restano rettangolari e allineati.
- Player, zombie e boss sono ancorati a `bottom_center`, senza pavimento
  incorporato nello sprite.
- Attraversare i cinque biomi e controllare center, seam, passage, cliff,
  mesa, prop e hazard a `1280x720` e `960x540`.
- Aprire menu, Character Select e mappa esplorativa a entrambe le risoluzioni.
...
./tools/run_visual_qa.ps1 -SkipImport -Filter biome
```
...
- La griglia top-down ortogonale e visibile.
...
- Verificare gli asset top-down con
  `godot --headless --path . --script res://tools/generate_top_down_environment_assets.gd -- --check`.
...
- Per release EXE, installare prima i template ufficiali Godot `4.6.3` in
  ...
  e `build/local_action_sandbox.exe --rendering-method gl_compatibility -- --build-smoke`.
...
## Regressione ISO-001 Milestone 2 - terrain isometrico 75x75
...
- Confermare che ogni regione sia `75x75` tile logici, equivalente a
...
- Verificare che la base `75x75` resti continua e che passaggi e fall zone
...
- Aprire `assets/environment/top_down/manifest.json` e confermare che versione
  e contratto di proiezione coincidano con il loader runtime.
...
- Avviare survival con seed fisso e confermare che il terreno visibile copra
  tutta la regione `75x75`, non solo il centro.
...
`BiomeTileResolver`, `BiomeTileLayer` o agli asset in
`assets/environment/top_down/tiles/forest/`.
...
Dettagli del contratto: `docs/forest_top_down_texture_system.md`.
...
- Eseguire `prepare_top_down_biome_assets.gd -- --check`: devono risultare
...
(`BiomeTileLayer`, `BiomeTileResolver`, manifest/asset catalog,
...
`BiomeTileResolver`, manifest o transizioni tra regioni.
...
- Verificare che le strade principali e le diramazioni verso i varchi usino
  segmenti orizzontali/verticali e curve fra lati cardinali.
...
`EnvironmentObject`, `SupplyCrateVisual`, manifest o asset
`object_scenes`.

- Avviare survival con seed fisso e verificare che rocce, case, barriere,
  relitti, tronchi, barili e muri appaiano come oggetti con base rettangolare,
  ombra e volume controllato, non come poligoni scollegati dal terreno.
...
- Attivare `F9`: i nove slot dell'albero e i rettangoli roccia `3x3`-`5x5`
...
- Con i template Godot `4.6.3` installati, il preset `Windows Desktop` genera `build/local_action_sandbox.exe`.
...
- In `Infinite Arena`, giocare almeno 20 minuti o 10 wave e verificare che il
  blocco `75x75` resti chiuso, senza world runtime, seam o softlock.
